...
module.exports = {
    ...
    beforeConstruct: function( self, options )
    {
        ...
        var categorySelectorTranslation =
        {
            name: 'categorySelectorTranslation',
            label: 'Category selector translation',
            fields: []
        };
        ...
        for( var idx = 0; idx < languages.length; idx++ )
        {
            ...
            var categorySelect =
            {
                name: 'categorySelect' + languages[ idx ],
                label: 'Category selection label ' + languages[ idx ],
                type: 'string'
            };
            fields.push( categorySelect );
            categorySelectorTranslation.fields.push( 'categorySelect' + languages[ idx ] );
        ...
        arrangements.push( categorySelectorTranslation );
    ...
    construct: function( self, options )
    {
        ...
        self.loadCategoryChoices = async function( language )
        {
        ...
        self.generateCategoryList = function( products, categories )
        {
            let categoryList = [];
            for( product of products )
            {
                let newCategory = categories.find( category => category.code === product.category );
                if( newCategory )
                {
                    let existingCategory = categoryList.find( category => category.id === newCategory.id );
                    if( existingCategory )
                    {
                        existingCategory.products.push( product.id );
                    }
                    else
                    {
                        newCategory.products = [ product.id ];
        ...
        self.load = ( req, widgets, callback ) => superLoad( req, widgets, async ( err ) =>
        {
            ...
            for( const widget of widgets )
            {
                ...
                const categories = await self.loadCategoryChoices( req.data.language );
                ...
                const categoryList = self.generateCategoryList( filteredProducts, categories );
                const datesList = self.generateDatesList( prices );

                widget.countryAreaList = countryAreaList;
                widget.categoryList = categoryList;
        ...
        {
            superPushAssets();
            self.pushAsset( 'script', 'searchbar', { when: 'always' } );
        };
    }
};
